...
{
    public sealed class GridContainerLayoutSystem : ISystem
    {
        private readonly List<LayoutMember> _members = new();

        public void Update()
        {
            foreach (var container in W.Query<All<DragContainer, GridContainerLayout, Position, ContainerLayoutDirty>>().Entities())
            {
                ref readonly var position = ref container.Read<Position>();
                ref readonly var layout = ref container.Read<GridContainerLayout>();
                ApplyLayout(container, position.Value, layout);
                container.Delete<ContainerLayoutDirty>();
            }
        }

        private void ApplyLayout(W.Entity container, Vector2 containerPosition, in GridContainerLayout layout)
        {
            _members.Clear();
