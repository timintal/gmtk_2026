namespace Xarbrough.SelectionUtility
{
	...
	internal class ListDisplay
	{
		...
		/// <summary>
		/// Horizontal offset added per hierarchy indent level in the overview list.
		/// </summary>
		...
		private void BindItem(VisualElement element, int index)
		{
			...
			// Rows are pooled/reused, so the indent must be (re)applied on every bind.
			element.style.paddingLeft = RowPadding / 2f + item.IndentLevel * IndentWidth;
