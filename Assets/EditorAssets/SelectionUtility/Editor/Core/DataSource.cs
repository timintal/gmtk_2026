namespace Xarbrough.SelectionUtility
{
	...
	internal class DataSource
	{
		...
		/// <summary>
		/// The deepest indent level present in <see cref="FilteredItems"/>. Used to reserve
		/// horizontal space for the hierarchy overview indentation.
		/// </summary>
		public int MaxIndentLevel { get; private set; }

		public DataSource(IList<GameObject> options)
		{
			Items = options;
			FilteredItems = Hierarchy.ArrangeAsOverview(options.Where(x => x != null).ToList());
			RefreshMaxIndentLevel();
		...
		{
			var matched = new List<GameObject>(Items.Count);
			...
			// Arrange the surviving items into a hierarchy overview (parents above their
			// descendants) rather than a flat, draw-order list that is hard to navigate.
			FilteredItems.Clear();
			FilteredItems.AddRange(Hierarchy.ArrangeAsOverview(matched));
			RefreshMaxIndentLevel();

			Changed?.Invoke();
		}

		private void RefreshMaxIndentLevel()
		{
			int max = 0;
			for (int i = 0; i < FilteredItems.Count; i++)
			{
				if (FilteredItems[i].IndentLevel > max)
					max = FilteredItems[i].IndentLevel;
			}
